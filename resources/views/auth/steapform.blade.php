...
@section('scripts')
    ...
    <script>
        ...
        function validateInputs(s){var ok=true;s.find("input:visible,select:visible,textarea:visible").each(function(i,e){if(e.hasAttribute('required')&&!e.checkValidity()){ok=false;e.classList.add('is-invalid')}else{e.classList.remove('is-invalid')}});if(!ok){var firstInvalid=s.find('.is-invalid:visible').first();if(firstInvalid.length){var label=firstInvalid.closest('.sip-field').find('label').text().trim();alert('Mohon lengkapi: '+label+' terlebih dahulu.');firstInvalid.focus()}}return ok}
