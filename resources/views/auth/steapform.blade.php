...
@section('scripts')
    ...
    <script>
        ...
        function validateInputs(s){var ok=true;var msg='';s.find("input,select,textarea").each(function(i,e){if(e.hasAttribute('required')&&!e.checkValidity()){ok=false;e.classList.add("is-invalid");var label=$(e).closest('.sip-field').find('label').text().trim();if(!msg)msg='Lengkapi: '+label+' terlebih dahulu.'}else{e.classList.remove("is-invalid")}});if(!ok&&msg){alert(msg)}return ok}
